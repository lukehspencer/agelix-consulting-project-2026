...
`POST /upload/predict-all` passes `rul_raw_days` (`round(prediction["rul_raw"] * 365)`) and `rul_calibrated` (`prediction["calibrated"]`) through on each asset result, alongside the existing `rul_months`-style precomputed fields. `DynamicAssetTable.jsx` shows a small ⚓ next to the number in the **ML Est.** column when `rul_calibrated` is true (tooltip: "Prediction anchored to this asset type's observed training range" -- calibration is specifically an ML-model concept, so the icon lives on the ML-only column, not the primary RUL Est. column) and a note in the explanation popup naming both the raw and calibrated ML day counts.
