...
calculate_mtbm(mtbf_days, risk_factor, current_interval_days=90, rul_days=None) -> dict
...
All three branches compare the rounded result to `current_interval_days` (see below for how this is resolved) to recommend `"shorten"` (< 80% of current), `"extend"` (> 120% of current, RUL-based and MTBF-based branches only), or `"maintain"`. Returns `{"mtbm_recommended_days", "current_interval_days", "recommendation", "recommendation_text", "next_maintenance_date", "basis", "rul_days_used"}` (`next_maintenance_date` is an ISO date, today + recommended days).
...
**Frontend (`DynamicAssetTable.jsx`):** the risk-ranking table has no MTBF column -- MTBF is popup-only, in a "Historical MTBF" card (title, `{mtbf_days} days` or `N/A*` value, confidence badge, the backend's dynamic `mtbf_note`, plus a static disclaimer: "Requires 2+ observed failures to calculate. Based on failure log history only."). What used to be a single "PM Interval" column with a shorten/extend/maintain arrow is now two plain columns: **PM Interval** (`{mtbm_recommended_days} d`, red if `recommendation === "shorten"`, green if `"maintain"` or `"extend"`, no arrow) and **Next PM Date** (`mtbm.next_maintenance_date` formatted `"MMM DD"`, red if due within 30 days, yellow within 90, green beyond, `N/A` if null). The popup's MTBM card is titled "Recommended MTBM" with a static sub-line ("Optimal interval based on asset risk and degradation rate"), a `mtbmBasisText(mtbm)` sub-line naming which of the three `calculate_mtbm()` branches produced the number ("Calculated from predicted RUL ({rul_days_used} days x 75%)" for `basis="rul_based"`, "Calculated from historical MTBF" for `"mtbf_based"`, "Adjusted from current interval based on risk level" for `"risk_adjusted"`), plus the existing `pm_interval_source`/`pm_interval_confidence`/`current_interval_days` sub-lines, the shorten/extend/maintain badge, and `recommendation_text`.
