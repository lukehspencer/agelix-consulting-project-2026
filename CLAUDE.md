...
`POST /upload/predict-all` passes `rul_raw_days` (`round(prediction["rul_raw"] * 365)`) and `rul_calibrated` (`prediction["calibrated"]`) through on each asset result, alongside the existing `rul_months`-style precomputed fields. `DynamicAssetTable.jsx` shows a small ⚓ next to the RUL pill when `rul_calibrated` is true (tooltip: "Prediction anchored to this asset type's observed training range") and a note in the explanation popup naming both the raw and calibrated day counts.
...
| DynamicAssetTable | DynamicAssetTable.jsx | Yes | Risk ranking sorted by health urgency (not raw risk factor) + Health Status column/badge + summary counts banner + RUL/MTBF/PM Interval columns (RUL pill shows a small ⚓ when `rul_calibrated` is true) + breach status column + urgency banner, and Explain / Breach Alerts popups (Multi-Sensor Analysis + Maintenance Planning with PM interval source/confidence + breach metrics + a calibration note naming the raw vs. calibrated day count when `rul_calibrated` is true). All urgency coloring (Health Status badge, Risk Factor/RUL pills, breach badge, MTBM recommendation) is sourced from one `COLORS` constant (`critical`/`at_risk`/`monitor`/`healthy`/`neutral`) so every visual element agrees on what's urgent |
