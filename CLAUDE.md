...
4. DynamicAssetTable (only rendered when predictedAssets.length > 0; Health Status column +
   summary counts banner, breach status column, RUL Est./ML Est./Deg. Proj./PM Interval/Next PM
   Date columns, two urgency banners (Critical in red, strictly Health-Status-driven; a broader
   "require scheduling" orange banner that also fires on unresolved breaches or a Next PM Date
   within 30 days), and per-asset Explain / Breach Alerts popups)
...
| DynamicAssetTable | DynamicAssetTable.jsx | Yes | Risk ranking sorted by health urgency (not raw risk factor), computed from `rul_days` -- the model-**selected** primary RUL (`rul/consensus_rul.py`'s `select_rul()`), not always the raw ML value. Health Status column/badge + summary counts banner + five RUL-related columns: **RUL Est. (days)** (the primary/selected value, colored badge, a colored source-label badge below it -- "ML" blue / "Deg. Proj." purple / "Avg" green -- plus a consensus badge: green "✓ Models agree" / yellow "~ Similar" / red "⚠ Models diverge"), **ML Est.** (plain grey number, no badge/color/emoji, ⚓ shown when `rul_calibrated` is true -- calibration is an ML-specific concept so the icon lives here, not on RUL Est.), **Deg. Proj.** (plain grey number from `rul_physics_days`, no badge/color/emoji -- deliberately styled as a secondary/input number, not a second urgency signal), **PM Interval** (red if `recommendation === "shorten"`, green if `"maintain"`/`"extend"`, no arrow), **Next PM Date** (`"MMM DD"`, red within 30 days / yellow within 90 / green beyond). No MTBF column in the table -- MTBF is popup-only. Breach status column + two urgency banners: red "immediate attention" strictly from `getHealthStatus()` (same as the Health Status column, so the two never disagree), and a broader orange "require scheduling" banner that also fires on an unresolved high/medium breach or a Next PM Date within 30 days -- an asset can appear in that banner while its own row still shows a Healthy/Monitor badge, which is intentional (breach/PM-date urgency is a different signal than RUL-health urgency), unlike the red banner's strict agreement guarantee. Explain popup adds, in order: a calibration note (raw vs. calibrated ML day count, when `rul_calibrated`), a "Primary RUL Estimate" breakdown (`rul_days`, source badge, `rul_reason`, and the ML/physics/consensus/confidence breakdown), Multi-Sensor Analysis, Maintenance Planning ("Historical MTBF" card with a static "requires 2+ failures" disclaimer, "Recommended MTBM" card), and a "Degradation Projection" section (per-sensor trend-to-threshold lines for sensors trending toward failure, the limiting sensor called out, an ML-vs-projection consensus line) -- plus Breach Alerts popups. All urgency coloring (Health Status badge, Risk Factor/RUL Est. pills, breach badge, MTBM recommendation, PM Interval/Next PM Date) is sourced from one `COLORS` constant (`critical`/`at_risk`/`monitor`/`healthy`/`neutral`); the ML Est./Deg. Proj. columns and their "lighter grey" labels are deliberately outside that system since they're plain secondary numbers, not urgency signals |
...
| The ML Est. and Deg. Proj. table columns are deliberately unstyled (plain grey number, no color/badge/emoji) | They're secondary/input values feeding the primary RUL Est. selection, not independent urgency signals -- giving them the same colored-badge treatment as RUL Est. would visually compete with it |
