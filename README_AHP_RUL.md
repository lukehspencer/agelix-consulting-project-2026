...
- **Training mode** -- the file is historical run-to-failure data with a known outcome per row. A fresh XGBoost model is trained and saved to `rul/models/<asset_type>.pkl`. The dashboard shows a green banner: "Training mode detected -- True_RUL_Days found," with the training sample count and test RMSE, and a note that predicting on new data requires a file *without* that column. Each asset's snapshot row is spread across 50-90% of its lifecycle (by its position among the uploaded assets) so a multi-asset training file shows meaningful variation in risk/RUL, rather than every asset being sampled at the same relative point.
- **Prediction mode** -- the file is current telemetry with no known outcome yet. No training happens. The system looks up the best-matching pre-trained model for the inferred asset type (exact name match first, then word-overlap match) and scores assets against it. The dashboard shows a blue banner: "Prediction mode -- using pre-trained model," naming the matched asset type and its feature count. If no matching model exists yet, the upload is rejected with the exact command needed to fix it. Each asset's snapshot is always its **most recent telemetry row** -- current state, not a lifecycle sample -- since a real deployment prediction should reflect where the asset actually is right now. This applies consistently to both `/upload/analyze`'s scoring preview and the RUL values `/upload/predict-all` returns for the dashboard.
...
When a prediction is calibrated, the asset's explanation popup shows a note naming both the raw (uncalibrated) and calibrated day counts (calibration is specifically about the ML model, so this note lives alongside the ML figure in the popup's "Model Details" breakdown, not on the headline RUL estimate -- see Degradation Projection and Model Selection below). The confidence interval shown alongside RUL is a fixed ±182-day (~6 month) band computed around whichever value is actually displayed as the headline RUL -- not a statistically derived interval, and not the raw ML model's own CI, but a more useful approximation given the RUL ranges typically seen in practice.
...
The risk ranking table shows a single **RUL Est.** column for this -- the primary/selected estimate, with a colored badge and a small label showing which model was selected (ML, Deg. Proj., or Avg) plus a consensus indicator showing how closely the two models agreed. The individual ML and degradation-projection figures are not separate table columns (an earlier version showed all three side by side; this competed for attention with the single primary number) -- they live in the asset explanation popup instead, inside a collapsible **"Model Details"** section that's collapsed by default behind a "Show model details" toggle. Expanding it shows exactly how the primary estimate was reached: the selected value and source, the reason it was selected, and the ML Model / Degradation Projection / Consensus / Physics confidence breakdown. Separately, the popup always shows a "Degradation Projection" section listing every sensor trending toward its threshold with a projected crossing date -- that section is per-sensor detail, distinct from the summary numbers in Model Details.
...
| POST | `/upload/predict-all` | AHP weights + manual scores -> RUL for all uploaded assets (400 if criteria have never been approved; weight-only re-runs after that don't need re-approval). Column-name lookups use the schema of the file actually being scored, not the model's original training file. Snapshot selection follows the same training/prediction split as `/upload/analyze` -- most recent row for a prediction-mode file, lifecycle-spread offset for a training-mode file |
